...
| Ticking the boxes | A normal Playwright click on the styled labels times out ("waiting for element to be visible, enabled and stable") and the inputs themselves are hidden, so the script dispatches a `click` event on `label[for=…]` and then verifies `is_checked()`. Ticking Manual Payment reveals a note: "Please scan the PayNow QR code from the Documents section in the iPlusLiving app or proceed to the Management Office to make payment." |
| **Proceed** | `a.btnpaymentsubmit` (the `input[type=submit]` is commented out). It opens a confirmation modal `#customConfirm` "Are you sure you want to proceed the booking?" with **Yes** = `button.confirm-ok` and No. Yes POSTs the form by AJAX and then `window.location = data.url`; an error shows an `iplus.alert` popup. Leaving the page (`beforeunload`) releases the slot lock, but the slot stays grey for roughly an hour afterwards. |
